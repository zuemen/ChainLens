...
## 第二輪：依獨立檢查報告修正（2026-09-17 深夜）
以「第一次打開網站的評審」視角做了一次獨立檢查，主要修正：

- **投影解析度**：舞台寬度改由視窗高度推算（`.replay-frame` 的 `max-width: calc((100vh - 352px) * 2.143)`），
  1280×720 也能一屏看完定位＋旁白＋整個舞台；算式面板字級用 `cqw` 跟著舞台縮放，不再壓到節點。
- **節奏**：每幕時間改為 3.8／3.4／4.2／5.2／4.8 秒（約 21 秒到結論）；前兩幕加「出金申請單」卡片交代情境；
  最後一幕把「名單比對：放行」（劃掉）與「鏈鏡：暫緩出金」並排。第二個出金地址加上標籤，旁白同步改成「流向兩個出金地址」。
- **三頁圖譜一致**：Cytoscape 配色改成與案件重演同一套語意（金＝被害人、橘＝風險、灰藍＝正常、米白＝目標），
  加圖例，只標關鍵節點名稱（12px）；工作台預設改載入同一份劇本圖（`/graph` 新增 `scenario` 模式）。
- **STR 草稿用語**：自身分數不再被稱為「綜合風險評分」；無標註圖的社群風險來自圖樣命中的代理標註，
  不再寫「已知非法佔比」；SNA 欄位名改成白話（收款來源數等）；第五節不宣稱系統會留存稽核軌跡，改為建議法遵人員留存。
- **公式透明**：審查頁與重演旁白寫出融合公式（1 −(1−自身)×(1−關聯)，關聯＝0.6^(階數−1)）。
- 其他：所有 Demo 入口帶 `?auto=1`；審查頁空狀態提示與對照組放行原因；工作台明寫公開站停用即時查詢；
  研究成果頁補定位說明、F1 長條圖、名詞說明與 GitHub 連結；手機版比較表改直式卡片；瀏覽器打錯網址導回首頁。
